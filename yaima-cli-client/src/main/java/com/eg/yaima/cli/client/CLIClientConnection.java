...
public class CLIClientConnection implements ClientConnection {
    private static final Logger LOGGER = LoggerFactory.getLogger(CLIClientConnection.class);

    private String ip;
    private int port;
    private Socket socket;
    private CLI uiHandler;
    private String username;

    public CLIClientConnection(String ip, int port) {
        this.ip = ip;
        this.port = port;
    }
        ...
        try {
            ...
            while(true) {
                ...
                if (packetType.equals("STT")) {
                    ...
                    uiHandler.getGUI().getGUIThread().invokeLater(() -> {
            ...
            LOGGER.error("ERR:", e);
            while(uiHandler.getGUI() == null) { // wait until lanterna is running
                try {
                    Thread.sleep(1000);
...
//        this.username = username;
    ...
    public void setUIHandler(UIHandler uiHandler) {
        if (uiHandler instanceof CLI)
            this.uiHandler = (CLI) uiHandler;
        else
